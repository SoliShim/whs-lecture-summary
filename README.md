...
5. 공개 페이지는 학습 목표, 상세 정리, 예시, 복습 질문, 강의 원문처럼 학생이 바로 공부하는 데 필요한 내용만 보여 준다.
6. 제작 방식과 정리 과정을 설명하는 별도 섹션은 만들지 않되, 학생이 실제 발화를 확인할 수 있는 강의 원문 보기 기능은 유지한다.
...
- `공통/개발 > 최신 보안 동향`: 7개 강의 HTML 생성 완료
- `공통/개발 > 해커의 프로그래밍`: 9개 강의 HTML 생성 완료
- `공통/개발 > 시큐어코딩`: 8개 강의 HTML 생성 완료
